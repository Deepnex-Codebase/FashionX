      ...
      <motion.div 
        ...
        {user && user.plan && (
          <div className="bg-[#f9f7f5] border border-[var(--almond)] text-[var(--coffee)] p-4 rounded-lg mb-8 max-w-xl mx-auto">
            <p className="font-medium text-lg">Current Plan: <span className="font-bold capitalize">{typeof user.plan === 'object' ? user.plan.name || 'Basic' : user.plan}</span></p>
      ...
      {plansLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 px-4 sm:px-6 lg:px-0">
          {plans.map((plan, index) => {
            ...
            // Ensure plan is properly formatted before passing to PricingCard
            const formattedPlan = {
              ...plan,
